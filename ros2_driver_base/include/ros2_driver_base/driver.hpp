#ifndef ROS2_DRIVER_BASE_DRIVER_HH
#define ROS2_DRIVER_BASE_DRIVER_HH
...
#include <vector>
#include <unistd.h>
#include <ros2_driver_base/status.hpp>
...
public:
    ...
    typedef ros2_driver_base::Status Statistics;
...
protected:
    ...
    /** Default read timeout for readPacket in milliseconds
     *
     * @see getReadTimeout setReadTimeout readPacket
     */
    std::chrono::milliseconds m_read_timeout;

    /** Default write timeout for writePacket in milliseconds
     *
...
public:
     ...
     */
    void setReadTimeout(std::chrono::milliseconds const& t);

    /** Get the default read timeout in milliseconds*/
    std::chrono::milliseconds getReadTimeout() const;

    /** Sets the default write timeout in milliseconds. Used in writePacket calls
     * without timeout parameters
     */
    void setWriteTimeout(std::chrono::milliseconds const& t);
    ...
    */
     ...
     */
    int readPacket(uint8_t* buffer, int bufsize, std::chrono::milliseconds const& packet_timeout);
     ...
     */
    int readPacket(uint8_t* buffer, int bufsize, std::chrono::milliseconds const& packet_timeout, std::chrono::milliseconds const& first_byte_timeout);
     ...
     */
    bool writePacket(uint8_t const* buffer, int bufsize, std::chrono::milliseconds const& timeout);
