...
#include <ros2_driver_base/timeout.hpp>
...
#include <ros2_driver_base/io_stream.hpp>
#include <ros2_driver_base/io_listener.hpp>
...
void Driver::setReadTimeout(std::chrono::milliseconds const& timeout)
{ m_read_timeout = timeout; }
std::chrono::milliseconds Driver::getReadTimeout() const
...
int Driver::readPacket(uint8_t* buffer, int buffer_size,
        std::chrono::milliseconds const& packet_timeout)
{
    return readPacket(buffer, buffer_size, packet_timeout,
            packet_timeout + std::chrono::milliseconds(1s));
}
int Driver::readPacket(uint8_t* buffer, int buffer_size,
        std::chrono::milliseconds const& packet_timeout, std::chrono::milliseconds const& first_byte_timeout)
{
    return readPacket(buffer, buffer_size, packet_timeout.count(),
            first_byte_timeout.count());
}
int Driver::readPacket(uint8_t* buffer, int buffer_size, int packet_timeout, int first_byte_timeout)
{
    ...
    if(!m_stream)
        throw std::runtime_error("Driver::readPacket : invalid stream, did you forget to call open ?");
    ...
    while(true) {
        ...
        try {
            ...
            // while-iteration)
            m_stream->waitRead(std::chrono::milliseconds(remaining_timeout));
        }
...
void Driver::setWriteTimeout(std::chrono::milliseconds const& timeout)
{ m_write_timeout = timeout; }
std::chrono::milliseconds Driver::getWriteTimeout() const
...
bool Driver::writePacket(uint8_t const* buffer, int buffer_size, std::chrono::milliseconds const& timeout)
{ return writePacket(buffer, buffer_size, timeout.count()); }
bool Driver::writePacket(uint8_t const* buffer, int buffer_size, int timeout)
{
    ...
    while(true) {
        ...
        m_stream->waitWrite(std::chrono::milliseconds(remaining_timeout));
    }
}
