...

{
    ...
    {
        // TODO FIXME: what / how to do this correct ?
        std::cout << "FD given to Driver::setFileDescriptor is set as blocking, setting the NONBLOCK flag" << std::endl;
        //RCLCPP_DEBUG( "FD given to Driver::setFileDescriptor is set as blocking, setting the NONBLOCK flag");
    }
}
FDStream::~FDStream()
{
...
void FDStream::waitRead(std::chrono::milliseconds const& timeout)
{
    fd_set set;
    FD_ZERO(&set);
    FD_SET(m_fd, &set);

    timeval timeout_spec = { static_cast<time_t>(std::chrono::duration_cast<std::chrono::seconds>(timeout).count()), suseconds_t(timeout.count() * 1000 % 1000000)};
...
void FDStream::waitWrite(std::chrono::milliseconds const& timeout)
{
    fd_set set;
    FD_ZERO(&set);
    FD_SET(m_fd, &set);
