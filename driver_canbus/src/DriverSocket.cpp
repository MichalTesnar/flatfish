...
//#include <ros2_driver_base.hh>

using namespace canbus;
using ros2_driver_base::UnixError;
using ros2_driver_base::TimeoutError;
using ros2_driver_base::Timeout;
...
bool DriverSocket::open(std::string const& path)
{
    ...
    ros2_driver_base::FileGuard guard(fd);
...
bool DriverSocket::checkInput(Timeout timeout)
{
    ...
    while(true) {
        ...
        if (res == -1 && errno != EAGAIN) 
            throw ros2_driver_base::UnixError("read(): error in recvmsg()");
        if (res >= 0) {
    ...
    if (haveTimestamp)
        result.can_time += std::chrono::seconds(tv.tv_sec) + std::chrono::microseconds(tv.tv_usec);
    else
...
Message DriverSocket::read()
{
    ...
    while (rx_queue.empty()) {
        if (!checkInput(timeout))
            throw ros2_driver_base::TimeoutError(ros2_driver_base::TimeoutError::PACKET, "read(): timeout");
    }
