...
Driver2Web::Driver2Web()
    : ros2_driver_base::Driver(CAN_MSG_SIZE_MIN + 16),
...
Message Driver2Web::read()
{
    ...
    m_status.time = std::chrono::system_clock::now();
    ...
    result.can_time += std::chrono::seconds(canMsg.secs) +  std::chrono::microseconds(canMsg.u_secs);
