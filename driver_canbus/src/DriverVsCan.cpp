#include <drivers_canbus/DriverVsCan.hpp>
...
bool DriverVsCan::reset_board()
{
    ...
    // timestampBase = std::chrono::system_clock::now();
...
bool DriverVsCan::checkForMessages(ros2_driver_base::Timeout& timeout)
{
#define BUFFER_SIZE 100
    ...
    while(rx_cnt < 1)
    {
        ...
        if((ret = VSCAN_Read(handle, buffer, BUFFER_SIZE, &read_cnt) != VSCAN_ERR_OK))
            throw ros2_driver_base::UnixError("read(): error in reading can messages");
        ...
        {        
            ...
            result.can_time += std::chrono::milliseconds(time);
            //  + timestampBase;
...
{
    ros2_driver_base::Timeout timeout(m_read_timeout);

    if(!checkForMessages(timeout))
        throw ros2_driver_base::TimeoutError(
                ros2_driver_base::TimeoutError::PACKET, "read(): timeout");
...
void DriverVsCan::write(Message const& msg)
{
    ...
    if(VSCAN_Write(handle, &out, 1, &out_cnt) != VSCAN_ERR_OK)
        throw ros2_driver_base::UnixError("write(): error during write");
}

int DriverVsCan::getPendingMessagesCount()
{
    ros2_driver_base::Timeout t(0);
    checkForMessages(t);
...
{
    return ros2_driver_base::Driver::INVALID_FD;
}
