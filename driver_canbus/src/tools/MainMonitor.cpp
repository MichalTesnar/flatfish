...
int main(int argc, char**argv)
{
    ...
    std::chrono::system_clock::time_point firstCanTime;
    while(count == -1 || i < count)
    {
        ...
        uint64_t timeDeltaUs = std::chrono::duration_cast<std::chrono::microseconds>(msg.time - firstTime).count();
        uint64_t canTimeDeltaUs = std::chrono::duration_cast<std::chrono::microseconds>(msg.can_time - firstCanTime).count();

        cout << dec << setw(10) << timeDeltaUs << " " << setw(10) << canTimeDeltaUs << " " << setw(10) << i << " " << setw(6) << hex << msg.can_id << " " << setw(4) << std::dec << (int)msg.size;
