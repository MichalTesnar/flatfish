...
DriverEasySYNC::DriverEasySYNC(int queue_size)
    : ros2_driver_base::Driver(MAX_PACKET_SIZE)
{
...
{
    ros2_driver_base::Driver::setWriteTimeout(std::chrono::milliseconds(timeout));
}

uint32_t DriverEasySYNC::getWriteTimeout() const
{
    return ros2_driver_base::Driver::getWriteTimeout().count();
}

void DriverEasySYNC::setReadTimeout(uint32_t timeout)
{
    ros2_driver_base::Driver::setReadTimeout(std::chrono::milliseconds(timeout));
}

uint32_t DriverEasySYNC::getReadTimeout() const
{
    return ros2_driver_base::Driver::getReadTimeout().count();
}
...
static void commandWithRetries(T lambda, int retries, int timeout) {
    //TODO: a timer should not be used like that
    std::chrono::system_clock::time_point deadline = std::chrono::system_clock::now() + std::chrono::milliseconds(timeout);

    while(true) {
        try {
            auto difference_ms = std::chrono::duration_cast<std::chrono::milliseconds>(deadline - std::chrono::system_clock::now());
            lambda(difference_ms.count());
            break;
...
int DriverEasySYNC::getPendingMessagesCount()
{
    ...
    catch(ros2_driver_base::TimeoutError&) {}
    return mQueue.size();
}
...
Message DriverEasySYNC::readFromIO(int timeout_ms)
{
    ...
    if (cursor + 4 == buffer + size)
    {
	    ...
	    message.can_time += std::chrono::milliseconds(can_time);
    }
...
{
    return ros2_driver_base::Driver::getFileDescriptor();
}

bool DriverEasySYNC::isValid() const
{
    return ros2_driver_base::Driver::isValid();
}

void DriverEasySYNC::close()
{
    processSimpleCommand("C\r", 2);
    ros2_driver_base::Driver::close();
}
...
DriverEasySYNC::Status DriverEasySYNC::getStatus()
{
    ...
    Status status;
    status.time = std::chrono::system_clock::now();
    if (raw_status & 0x20) {
        status.tx_state = OFF;
    }
...
int DriverEasySYNC::readReply(char command, uint8_t* buffer, int timeout)
{
    ...
    while(true) {
        auto difference_ms = (deadline - std::chrono::system_clock::now());
        int size = readPacket(buffer, MAX_PACKET_SIZE, std::chrono::duration_cast<std::chrono::milliseconds>(difference_ms).count());
        if (buffer[0] == '\x7')
