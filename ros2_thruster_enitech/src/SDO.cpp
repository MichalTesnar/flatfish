#include <thruster_enitech/SDO.hpp>
...
WriteHeartbeatPeriod::WriteHeartbeatPeriod(Protocol& protocol, std::chrono::milliseconds const& period)
    : Write(protocol, 0x1017, 0x00, static_cast<uint16_t>(period.count())) {}

WriteUpdatePeriod::WriteUpdatePeriod(Protocol& protocol, std::chrono::milliseconds const& period)
    : Write(protocol, 0x2200, 0x00, static_cast<uint16_t>(period.count())) {}
