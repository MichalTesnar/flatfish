#include <cstring>
#include <thruster_enitech/Protocol.hpp>
...
std::chrono::system_clock::time_point Protocol::getLastHeartbeat() const
{
...
{
    return mLastKnownStatus.time.time_since_epoch() != 0s;
}

Status Protocol::getLastStatus() const
{
    if (mLastKnownStatus.time.time_since_epoch() == 0s)
...
canbus::Message Protocol::makeCommand(const double value) const
{
    ...
    // NOTE: we removed the option to control the thruster in the raw mode
    // since it is not possible to set the mode anyway 
    // if (control_mode == Status::CONTROL_MODE::RAW)
    // {
    //     setvalue = static_cast<uint16_t>(fabs(value));
    //     command_byte |= 1 << 3;
    // }
    // else if (control_mode == Status::CONTROL_MODE::SPEED)
    // {
    //     setvalue = static_cast<uint16_t>(round(fabs(value) / (2 * M_PI)));
    // }
...
void Protocol::parsePDO(canbus::Message const& msg)
{
    ...
    // rpm to rd/s
    parsed.speed   = direction * static_cast<double>(read16(msg, 0)) * M_PI * 2 / 10.;
    parsed.current = direction * static_cast<double>(read16(msg, 2));

    parsed.start_gain = start_gain;
    parsed.air_parameters = air_parameters;
    parsed.control_mode = current_control ? Status::CONTROL_MODE::RAW : Status::CONTROL_MODE::SPEED;
